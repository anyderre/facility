...
public class RoomResource {
    ...
    public RoomResource(RoomRepository roomRepository, UserRepository userRepository) {
     ...
     */
    @PostMapping("/rooms")
    public ResponseEntity<Room> createRoom(@Valid @RequestBody Room room, Principal principal) throws URISyntaxException {
        ...
        user.ifPresent(room::setUser);
        room.creationDate(Instant.now());
        room.modificationDate(Instant.now());
    ...
    public ResponseEntity<List<Room>> getAllRooms(Pageable pageable) {
        ...
        Page<Room> page = roomRepository.findByUserIsCurrentUser(pageable);
